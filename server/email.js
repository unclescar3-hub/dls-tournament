...
async function sendAdminInviteEmail(recipientEmail, inviterName, inviteToken) {
  ...
  return sendEmail({
    ...
    html: baseWrapper(`
      ...
      <p style="color:#6b7a99;line-height:1.7"><strong style="color:#f0f4ff">${inviterName}</strong> has invited you to become a co-admin of <strong style="color:#00e676">Unclescar Studios</strong>. You'll be able to manage tournaments, review matches, and run the platform.</p>
      <div style="text-align:center;margin:28px 0">
      ...
      <p style="color:#6b7a99;font-size:12px">This invite expires in <strong>24 hours</strong>. If you don't have an account yet, you'll be prompted to create one.</p>`)
  });
}
  ...
  return sendEmail({
    ...
    html: baseWrapper(`
      ...
      <div style="margin-top:20px;text-align:center">
        <a href="${APP_URL()}/admin.html" style="background:#3b82f6;color:#fff;padding:11px 22px;border-radius:6px;text-decoration:none;font-weight:600">Open Admin Panel</a>
      </div>`)
  });
}
...
async function sendAdminMatchNotification(submitter, opponent, tournament, result) {
  return sendEmail({
    ...
    html: baseWrapper(`
      ...
      <div style="margin-top:20px;text-align:center">
        <a href="${APP_URL()}/admin.html" style="background:#ff1a5e;color:#fff;padding:11px 22px;border-radius:6px;text-decoration:none;font-weight:600">Review Now</a>
      </div>`)
  });
}
...
async function sendPayoutEmail(user, tournamentName, amount, position, accountName, bankName) {
  ...
  return sendEmail({
    to: user.email,
    subject: `💰 Prize Payout Sent — ${tournamentName}`,
    html: baseWrapper(`
      ...
      <div style="background:#0f1623;border:1px solid #1e2d4560;border-radius:8px;padding:14px">
        <p style="color:#6b7a99;margin:0;font-size:12px;line-height:1.7">Funds typically arrive within minutes via Paystack. If you don't receive within 24 hours, reply to this email.<br><br>Keep competing to earn more — check open tournaments now!</p>
      </div>
